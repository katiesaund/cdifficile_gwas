...
results_df <- read_tsv("../../../data/15_pyseer/paloc_model_results//EN_model_summary.tsv", col_names = TRUE)
...
ggsave(paste0("../../../figures/pyseer_results/paloc_EN_model_performance_jitter_all_pheno.png"), 
...
write_tsv(best_alpha_df, "../../../data/15_pyseer/paloc_model_results/best_alpha_values.tsv")
# We observe that the best alpha is tied: 0.745 and 0.99

# Let's plot just when the Rsq is the best - specific value for each phenotype
best_results_df <- results_df %>%  filter(alpha == 0.745) 
...
ggsave(paste0("../../../figures/pyseer_results/paloc_EN_model_performance_jitter_best_alpha_all_pheno.png"), 
       units = "in", height = 8.5, width = 7)

write_tsv(best_results_df, "../../../data/15_pyseer/paloc_model_results/best_model_results.tsv", col_names = TRUE)
