...
# The control stuff is made in the script: 1_make_neg_controls.R
...
# SNP + Indel + accessory 
snp_indel_pan_path <- "../../../data/15_pyseer/genotypes_for_pyseer/"
...
genotype_path_vec <- snp_indel_pan_path
names(genotype_path_vec) <- "snp_indel_pangenome" # c("snp", "indel", "pangenome", "snp_indel_pangenome")

# Phenotypes
ctrl_dir_path <- "../../../data/4_phenotypes/negative_controls/"
data_dir_path <-  "../../../data/15_pyseer/phenotypes_for_pyseer/"

true_pheno_paths <- paste0(data_dir_path, 
                      c("cleaned_fqR.tsv", 
                        "cleaned_log_cfe.tsv",
...
names(true_pheno_paths) <- c("fqR", "log_cfe", "log_germ_tc_and_gly", 
    ...
    for (k in 1:num_lineage) {
      for (l in 1:num_genotype) {
       ...
       #    for (n in 1:num_covariate) {
         ...
         #                        names(covariate_vec)[n])
            out_name <- paste0(model_name, ".out")
            
            
            geno_mat_path <- paste0(genotype_path_vec[l],
                                    "pyseer_cleaned_",
                                    names(pheno_paths)[m],
...
pyseer --phenotypes ",  
              ...
              " --pres ", 
              geno_mat_path,
              " --wg enet --save-model  ../../../data/15_pyseer/wgs_model_results/", 
              model_name, 
              " --alpha ", 
              alpha_value_vec[j], 
              " --n-fold ", 
              ...
              lineage_vec[k], 
              # covariate_vec[n],
              " > ../../../data/15_pyseer/wgs_model_results/", 
              model_name, 
              ".txt ")
