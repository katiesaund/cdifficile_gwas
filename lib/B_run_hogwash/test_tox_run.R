...
genotype <- genotype[, 2:400]
storage.mode(genotype) <- "numeric"

tr  <- ape::read.tree("../../data//3_trees/log_toxin.tree")
f_name <- "test"
out_dir <- "../../data//5_hogwash/log_toxin/"
perm_num <- 1000
fdr_value <- as.numeric(0.3)
bootstrap_threshold <- as.numeric(0.90)
test_type <- as.character("both")

group_method <- as.character("post-ar")

strain_key <- read.table("../../data//6_ribotype/log_toxin_ribotype_no_colors.tsv",
                         sep = "\t",
                         row.names = 1,
                         header = TRUE,
                         stringsAsFactors = FALSE,
